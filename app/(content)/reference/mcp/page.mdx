export const metadata = {
  title: "MCP server",
  description: "The ten tools, seven resources and three prompts Ripar exposes over the Model Context Protocol.",
};
...
| --- | --- | --- |
| Identity | `768633998` | Agent id → domain → controlling address |
| Reputation | `768633999` | Payments credited, USDC volume, validator verdicts |
| Validation | `768634000` | The job board and its verdicts |
...
Refuses before a fee is spent when the sender is not the client, the job is past funding,
...
Release needs a passing verdict, and may be called by the client — or by **anyone** once
the dispute window has passed, because a validator who never returns would otherwise
freeze the worker's money. Refund needs a failed verdict or a cancelled job, and pays the
client whoever triggers it: the destination is read off the job, so calling it cannot
redirect it.
...
Each references only tools that exist — asserted in the test suite, because a prompt
naming a missing tool fails at the point somebody trusts it.
