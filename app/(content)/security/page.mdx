...
- **On-chain reputation.** A credit requires the settling USDC transfer to be a
  transaction in the same atomic group, sent **from** the client's registered address
  **to** the server's — both resolved through the IdentityRegistry. So the amount is read
  off a transfer the AVM has already validated and cannot be fabricated. What a score
  still does *not* tell you is whether the work was any good: it counts money that moved,
  not results anyone judged. A validator verdict is a separate field.
