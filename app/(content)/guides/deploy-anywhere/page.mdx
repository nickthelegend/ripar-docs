...
[`@ripar/sdk`](/reference/sdk) is an ordinary Node HTTP server, so it runs anywhere
Node runs.
...
Ranking then reads the same signals as a managed endpoint: success rate, latency at p50
and p95, price against comparable endpoints, and settled volume. The difference is that
uptime is now yours to hold up, and a listing that times out ranks accordingly.

<Cards>
  <Card title="SDK reference" href="/reference/sdk">
    Every option on defineEndpoint, defineAgent, serve and RiparClient.
  </Card>
  <Card title="Limits & pricing" href="/limits">
