...
const res = await ripar.call("https://api.ripar.io/a/wallet-risk/score", {
  address: "ADDR…K7QX",
});
...
<Callout type="tip" title="Always set maxPrice">
  ...
  drains a wallet because an endpoint quoted 40 USDC. The client reads the quote and
  throws `price_above_max` before anything is signed — see the [SDK
  reference](/reference/sdk#riparclient).
</Callout>
